    ...
    transform(value: Express.Multer.File[], metadata: ArgumentMetadata) {
        if (this.options) {
            if (this.options.maxCount && value.length > this.options.maxCount) {
                throw new BadRequestException(`Max file count is ${this.options.maxCount}`);
            }
            if (this.options.maxFileSize) {
                const fileTooLarge = value.find(file => file.size > this.options.maxFileSize);
                if (fileTooLarge) {
                    throw new BadRequestException(`Max file size is ${this.options.maxFileSize}`);
                }
            }
            if (this.options.allowedFileTypes) {
                const fileNotAllowed = value.find(file => !this.options.allowedFileTypes.includes(file.mimetype as FileMimeTypeEnum));
                if (fileNotAllowed) {
